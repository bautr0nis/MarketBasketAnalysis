...
bank.rules <- apriori(transactions, parameter = list(support = 0.005, confidence = 0.3, target = "rules", minlen = 3))
...
sequences2 <- read_baskets("sequences_data.csv", sep = ",", info = c("sequenceID", "eventID", "SIZE"))

# Naudojame cspade algoritmą su žemu support lygiu sekų taisyklėms išgauti
s6 <- cspade(sequences2, parameter = list(support = 0.0133), control = list(verbose = TRUE, timeout = 15))
...
itemsets <- eclat(transactions,
                  parameter = list(supp = 0.005, minlen = 3))

# Print the frequent itemsets found by Eclat
itemsets

# Create rules from the frequent itemsets
eclat_rules <- ruleInduction(itemsets, transactions, confidence = 0.3)
eclat_rules
...
# Apibreziu skirtingus algoritmus palyginimui su asociacijų taisyklėmis
algorithms <- list(
  `AR conf = 0.1` = list(name = "AR", param = list(supp = 0.005, conf = 0.1)),
  `AR conf = 0.2` = list(name = "AR", param = list(supp = 0.01, conf = 0.2)),
  `AR conf = 0.05` = list(name = "AR", param = list(supp = 0.001, conf = 0.05)),
  `AR conf = 0.3` = list(name = "AR", param = list(supp = 0.05, conf = 0.3)),
  `AR conf = 0.4` = list(name = "AR", param = list(supp = 0.02, conf = 0.4))
)
